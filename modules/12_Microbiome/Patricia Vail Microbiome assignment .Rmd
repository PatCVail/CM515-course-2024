...
Metagenomics: known as shotgun sequencing is a way of getting all of the sequence information of an entire genome or an entire sample. The method random breaks the entire genome into fragments, perform sequencing, and then reassemble. This gives all of the diversity information within a sample which is useful for microbiome where there are many types of organisms present. The drawback is its cost.

Amplicon Sequencing: This is based on specific primers so will only sequence the organisms with the specific sequences encoded on the primers. This is useful for identifying the sequence of a specific part of the sample, and is cost effective therfore. It is limited in the information and diversity of the sample. 
...
Sample-ID, bar code, mouse_id, genotype, cage is, donor, donor status, post transplant, genotype and donor status. 

Thorough metadata ensures that you can link your sequence information to the sample and that you can run different analysis based on the information in your metadata. This is especially important for sharing your data later, and having your results make sense based on the experimental procedures. 


4.  What is the statistical test for pairwise comparison between the groups in the plots?

Wilcox pairwise test and Kruskal-Wallis

5.  Which comparisons were statistically significant from Shannon's alpha diversity?

Susceptible & Healthy vs Susceptible & PD : p-value = .00013
Wild Type & Healthy vs Susceptible & PD: p-value = .0015
...
This is a method to take samples with different depths of sequence information and make it standard to be able to compare effectively. You might lose some sequence information from samples with especially deep sequencing but it gives you the ability to compare all the diversity within a sample and make inferences. 


7.  What does beta diversity show that alpha diversity doesn't?

Beta diversity measures diversity between samples and accross environments . Alpha diversity only shows diversity in one environment. 

8.  What is ANCOM-BC used for and why did we use it in this class? 

ANCOM-BC is a statistcal method used to tell the abundance of something within microbiome sequence data. This can tell you the abundance of taxa or organisms in relation to each other. 
...
This shows that the least abundant was Ruminococcacea and the Bacteroides was the most. 